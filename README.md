...
### Local Secrets & Ignored Files

To keep your secrets safe, the following files are ignored by git:

- `.env` files (generated by `azd` or created manually)
- `appsettings.local.json` (for backend local overrides)
- `secrets.json` (if used for local secrets)
- `*.user` (user-specific project settings)

**Recommendation**: Use `.env` files for local environment variables. If you need to override backend settings locally, create `backend/WebApp.Api/appsettings.local.json`.
